...
class TokenManager:
    ...
    _ACCESS_TOKEN_EXPIRE_SECOND = config.ACCESS_TOKEN_EXPIRE_SECOND()

    @classmethod
    def create(cls, data: dict) -> str:
        to_encode = data.copy()
        expire = datetime.datetime.now(tz=timezone.utc) + timedelta(seconds=cls._ACCESS_TOKEN_EXPIRE_SECOND)
